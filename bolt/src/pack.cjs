...
function validateConfig(config) {
  if (
    (config.packageType === "base" ||
      config.packageType === "runtime" ||
...
async function pack(configFile, content) {
  ...
  if (exec(`which ralfpack >/dev/null; echo $?`).trim() === "0") {
    exec(
      `ralfpack create --config ${configFile} --content ${content} --image-format erofs.lz4 ${output}.bolt`,
    );
  } else {
  ...
  console.log(
    `Prepared ${output}.bolt package from ${configFile} and ${content}`,
  );
}
...
async function packInternal(content, config, output) {
  ...
  exec(
    `mkfs.erofs -b 4096 -C 4096 --ignore-mtime -E ^dedupe,^all-fragments,inline_data,^legacy-compress,^xattr-name-filter,^ztailpacking -zlz4 --all-root --tar --gzip ${erofsTmpFile} ${content}`,
  );
  const erofsTmpFileStat = statSync(erofsTmpFile);
  const verityInfo = exec(
    `veritysetup format ${erofsTmpFile} ${erofsTmpFile} --hash-offset=${erofsTmpFileStat.size}`,
  )
    .trim()
    .split("\n");
  let rootHash;
  ...
  if (
    !(
      ...
      rootHash.match(/^[0-9a-f]+$/) &&
      typeof salt === "string" &&
      salt.match(/^[0-9a-f]+$/)
    )
  ) {
    console.error(
      `Cannot find "Root hash" and/or "Salt" in veritysetup command output!`,
    );
    process.exit(-1);
  }
